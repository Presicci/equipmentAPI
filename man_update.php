...
$sql="Select * from `manufacturer` where `name`='Hisense'";
$result=$dblink->query($sql) or
	die("Something went wrong with: $sql<br>".$dblink->error);
$count=0;
while ($item=$result->fetch_array(MYSQLI_ASSOC))
{
	$sql="Select * from `equipment_prod` where `manufacturer`='$item[name]' limit 250000";
	$rst=$dblink->query($sql) or
		die("Something went wrong with: $sql<br>".$dblink->error);
	while($data=$rst->fetch_array(MYSQLI_ASSOC))
	{
		//echo "<p>About to update $data[auto_id] with new type:$item[name] from $data[type]</p>";
		$sql="Update `equipment_prod` set `manufacturer`='$item[auto_id]' where `auto_id`='$data[auto_id]'";
		$dblink->query($sql) or
			die("Something went wrong with: $sql<br>".$dblink->error);
		$count++;
	}
